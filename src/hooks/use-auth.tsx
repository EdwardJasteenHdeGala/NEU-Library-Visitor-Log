...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const fetchOrCreateProfile = async (uid: string) => {
    ...
    try {
      ...
      // CHECK FOR PRE-ADDED USER BY EMAIL (THE "GDRIVE" CLAIMING MECHANISM)
      if (!docSnap.exists() && user.email) {
        ...
        if (!qSnap.empty) {
          ...
          // Migrate pre-added placeholder profile to UID-indexed official profile
          const migratedData = {
            ...preData,
            id: user.uid,
          ...
          await deleteDoc(preDoc.ref); // Remove the placeholder
  ...
  const addUserByEmail = async (email: string, role: 'user' | 'admin' | 'guest', college: string) => {
    ...
    try {
      ...
      // Add as a placeholder profile. ID will be generated by Firestore.
      ...
      toast({
        ...
        description: `Access for ${email} has been configured. They will inherit this role upon first login.`,
      });
      return true;
    } catch (error: any) {
      toast({
