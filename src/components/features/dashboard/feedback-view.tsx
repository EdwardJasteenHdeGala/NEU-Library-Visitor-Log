...
import { diagnosticSession } from "@/lib/diagnostics/session";
...
export function FeedbackView({ onBack }: FeedbackViewProps) {
  ...
  const feedbackQuery = useMemoFirebase(() => {
    ...
    if (isAdmin) {
      return query(collection(firestore, 'feedback'), orderBy('timestamp', 'desc'), limit(100));
    }
    return query(
    ...
    <div className="grid grid-cols-1 gap-6">
      {isLoadingFeedback ? (
        ...
        <EmptyState 
          ...
          message="Institutional feedback telemetry will appear here once captured from the registry." 
        />
      ) : (
        feedbackList?.map((item) => (
          <Card key={item.id} className="neu-card-shadow border-none overflow-hidden hover:scale-[1.01] transition-transform bg-card rounded-[2rem]">
  ...
  const renderFeedbackForm = () => (
    <Card className="neu-card-shadow border-none rounded-[3rem] overflow-hidden bg-card max-w-2xl mx-auto shadow-2xl group/form">
      <CardHeader className="bg-[#032e41] p-12 text-center text-white relative">
         ...
         <div className="flex justify-center gap-4 mb-6 relative z-10">
            {[...Array(5)].map((_, i) => (
              <button 
                ...
                className="hover:scale-125 transition-all duration-300 drop-shadow-xl group-hover/form:rotate-3"
              >
                <Star className={`h-14 w-14 ${i < parseInt(rating) ? 'fill-secondary text-secondary' : 'text-white/20'}`} />
              </button>
            ))}
         </div>
         <CardTitle className="text-3xl font-black italic tracking-tighter uppercase relative z-10 drop-shadow-md">Rate Your Experience</CardTitle>
      </CardHeader>
      <CardContent className="p-10 md:p-16 space-y-12 bg-card">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
          <div className="space-y-4">
              ...
              <Select value={rating} onValueChange={setRating}>
                  <SelectTrigger className="h-16 rounded-2xl border-2 font-black focus:ring-primary shadow-inner text-lg bg-muted/20">
                      <SelectValue />
                  </SelectTrigger>
      ...
      {isAdmin ? (
        <div className="space-y-[2rem]">
          <div className="space-y-[0.5rem]">
            <h2 className="text-[2.5rem] md:text-[3.5rem] font-black text-primary italic uppercase tracking-tighter leading-none text-glow-primary">System Sentiments</h2>
            <p className="text-[0.625rem] md:text-[0.875rem] text-muted-foreground font-black uppercase tracking-[0.4em] opacity-40 leading-none">Review and monitor institutional experience telemetry.</p>
          </div>
          ...
          <Tabs defaultValue="list" className="w-full">
            <TabsList className="grid w-full grid-cols-2 h-18 bg-muted/40 p-1.5 rounded-[2rem] mb-12 shadow-inner border border-muted/50">
              <TabsTrigger value="list" className="rounded-2xl font-black text-[11px] uppercase tracking-[0.4em] gap-4 data-[state=active]:bg-card data-[state=active]:text-primary data-[state=active]:shadow-xl transition-all h-full">
                <LayoutList className="h-6 w-6" />
                Sentiment Board
              </TabsTrigger>
              <TabsTrigger value="form" className="rounded-2xl font-black text-[11px] uppercase tracking-[0.4em] gap-4 data-[state=active]:bg-card data-[state=active]:text-primary data-[state=active]:shadow-xl transition-all h-full">
