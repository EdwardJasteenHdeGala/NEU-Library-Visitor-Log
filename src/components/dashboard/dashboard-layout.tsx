...
export function DashboardLayout() {
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col overflow-hidden neu-mesh-gradient">
          ...
          <div className="hidden lg:flex items-center gap-1.5 p-1 bg-white/5 rounded-2xl border border-white/10 backdrop-blur-sm">
            {navItems.map((item) => (
                <button
                    ...
                    className={cn(
                        "px-5 py-2.5 text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-2 rounded-xl",
                        currentView === item.id 
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-72 rounded-[2rem] p-3 shadow-2xl border-none mt-4 bg-white/95 backdrop-blur-2xl animate-in slide-in-from-top-2 duration-300">
                ...
                {profile?.isAuthorizedAdmin && (
                  <DropdownMenuItem onClick={() => switchRole(profile.role === 'admin' ? 'user' : 'admin')} className="rounded-2xl h-14 gap-4 text-blue-600 focus:bg-blue-50 cursor-pointer px-5 transition-all">
                    <ShieldCheck className="h-5 w-5" />
                    <span className="font-black text-xs uppercase tracking-widest">Switch to User Mode</span>
                  </DropdownMenuItem>
                )}
                <DropdownMenuSeparator className="mx-3" />
                <DropdownMenuItem onClick={logout} className="rounded-2xl h-14 gap-4 focus:bg-muted cursor-pointer px-5 transition-all">
                  <RefreshCcw className="h-5 w-5 text-muted-foreground" />
                  <span className="font-bold text-sm text-muted-foreground">Switch Account</span>
                </DropdownMenuItem>
                <DropdownMenuItem onClick={logout} className="rounded-2xl h-14 gap-4 text-destructive focus:bg-destructive/5 cursor-pointer px-5 transition-all">
            ...
            <div className="lg:hidden bg-primary p-6 border-t border-white/10 shadow-2xl space-y-3 animate-in slide-in-from-top-4 duration-300">
                {navItems.map((item) => (
                    <button
                        ...
                        className={cn(
                            "w-full flex items-center gap-5 p-5 rounded-[1.5rem] font-black text-sm uppercase tracking-[0.2em] transition-all",
                            currentView === item.id ? "bg-secondary text-primary shadow-lg scale-105" : "text-white/70 hover:bg-white/10"
                        )}
                    >
      ...
      <main className="relative flex-1 w-full max-w-[1600px] mx-auto p-6 md:p-10 lg:p-12 animate-in fade-in duration-1000">
      ...
      <footer className="relative z-10 p-12 bg-white/40 backdrop-blur-md border-t flex flex-col items-center gap-8 mt-auto">
        <div className="flex items-center gap-5 opacity-40 grayscale hover:grayscale-0 hover:opacity-100 transition-all duration-700 cursor-default group">
            ...
            <div className="flex flex-col items-start -space-y-1">
              <span className="font-black text-lg uppercase tracking-[0.4em] text-primary">New Era University</span>
              <span className="text-[10px] font-black uppercase tracking-[0.3em] opacity-70">Institutional Management</span>
            </div>
        </div>
        <p className="text-[10px] font-black uppercase tracking-[0.6em] text-muted-foreground/30 text-center animate-pulse">
