...
import { Star, MessageSquare, Loader2, Send, History, User } from "lucide-react";
...
export function FeedbackView() {
  ...
  const feedbackQuery = useMemoFirebase(() => {
    return isAdmin ? query(collection(firestore, 'feedback'), orderBy('timestamp', 'desc')) : null;
  }, [firestore, isAdmin]);
  ...
  const renderAdminView = () => (
    <div className="space-y-8 animate-in fade-in duration-500">
      <div>
        <h2 className="text-3xl font-black text-primary mb-2 italic uppercase">User Sentiments</h2>
        <p className="text-muted-foreground font-medium">Review institutional feedback and service ratings.</p>
      </div>
  ...
  const renderUserView = () => (
    <div className="max-w-2xl mx-auto space-y-8 animate-in fade-in duration-500">
      ...
      <Card className="neu-card-shadow border-none rounded-[2rem] overflow-hidden bg-white">
